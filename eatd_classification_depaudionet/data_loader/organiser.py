...
def get_lengths(data, labels, config):
    ...
    for p in range(len(labels[0])):
        d = data[p][0]
        if exp_type == 'mel' or exp_type == 'mfcc':
...
def run_train(config, logger, checkpoint, features_dir, data_saver, val=True):
    ...
    def data_to_array(file):
        data = util.eatd_csv_read(file)
        data_array = np.array([[d[0] for d in data], [d[1] for d in data],
                               [d[2] for d in data], [d[3] for d in data]]).astype(int)
        data_array2 = np.array([[d[0] for d in data], [d[1] for d in data],
    ...
    comp_data = [j for i in [train_data, dev_data] for j in i]
    #import pdb
    #pdb.set_trace()
    #for i in range(len(comp_data)):
    #    elements = comp_data[i]
    #    elements[0] = float(elements[0])
    #    comp_data[i] = elements
    ...
    return generator, class_data, train_labels[0]
