...
# mel+cnn-lstm
from exp_run.models_pytorch import CustomMel
from exp_run import config_1 as config



if 'lrf' in config.EXPERIMENT_DETAILS.keys():
    learn_rate_factor = config.EXPERIMENT_DETAILS['lrf']
else:
    learn_rate_factor = 2
#learn_rate_factor=2
EPS = 1e-12
...
def forward(model, generate_dev, data_type):
    ...
    for data in generate_dev:
        (batch_data, batch_label, batch_folder, batch_locator, batch_indices) = data
    ...
    results_dict['indices'] = np.array(indices)
...
def evaluate_val_test(model, generator, data_type, class_weights, comp_res, class_num,
             f_score_average, epochs, logger, gender_balance=False,
             hidden_test=False):
    """
    Processes the validation set by creating batches, passing these through
    ...
    for p, fol in enumerate(folders):
        if fol not in collected_output.keys():
            collected_targets[fol] = targets[p]
            output_for_loss[fol] = outputs[p]
            #if outputs[p] <=0.4 or outputs[p]>=0.6:
            collected_output[fol] = np.round(outputs[p])
            counter[fol] = 1
        else:
            output_for_loss[fol] = outputs[p]
    ...
    if not hidden_test:
        complete_results, per_epoch_pred = prediction_and_accuracy(outputs,
                                                                   ...
                                                                   f_score_average)
        return complete_results, (per_epoch_pred, intermediate_outputs, \
                   intermediate_counter, intermediate_targets), results_dict
    else:
        return -1, (-1, intermediate_outputs, intermediate_counter, -1)
...
def logging_info(current_dir, data_type=''):
    ...
    if mode == 'test':
        ...
        #add debug directory for testing scripts experiments
        log_path = current_dir + '/' + pm[prediction_metric]
        if data_type == 'test':
            log_path = log_path + '_test_'+threshold+'.log'
        elif data_type == 'dev':
            log_path = log_path + '_val_test_'+threshold+'.log'
    else:
...
def setup(current_dir, model_dir, exp_num,data_type='', path_to_logger_for_test=None):
    """
    ...
    # THIS WILL DELETE EVERYTHING IN THE CURRENT WORKSPACE #i
    if os.path.exists(current_dir) and os.path.exists(model_dir) and mode != 'train':
        temp_dirs = os.listdir(model_dir)
        temp_dirs = natsort.natsorted(temp_dirs, reverse=True)
        if threshold == 'total':
            with open(model_dir + '/best_scores_total.pickle','rb') as f:
                current_epoch = pickle.load(f)[-1]
        elif threshold == 'fscore':
            with open(model_dir + '/best_scores_fscore.pickle','rb') as f:
                current_epoch = pickle.load(f)[-1]
        else:
            current_epoch = model_index_list[exp_num]
        ...
        os.mkdir(model_dir)
    elif os.path.exists(current_dir) and os.path.exists(current_dir+'daic') and mode == 'train':
        print('System exist, experiments are done')
        os._exit(1)
...
def record_top_results(current_results, scores, epoch, use_train_threshold, threshold):
    """
    ...
    if use_train_threshold:
        if current_results[8] > .86:       #might cause performance discrepancy
            trigger = True
    else:
        trigger = True
    if trigger:
        if validate:
            ...
            if threshold == 'total':
                ...
                if current_results[-6] >= scores[11]:    
                    best_res = [total, current_results[8], current_results[0],
                            current_results[1], current_results[9],
                            current_results[6], current_results[7],
                            current_results[10], current_results[23],
...
def update_complete_results(complete_results, avg_counter, placeholder,
    ...
    """
    complete_results_copy = complete_results.copy()
    complete_results_copy[0:11] = complete_results_copy[0:11] / avg_counter
    # Accuracy Mean
    complete_results_copy[8] = np.mean(complete_results_copy[0:2])
    complete_results_copy[23] = np.mean(complete_results_copy[15:17])
    # FScore Mean
    complete_results_copy[9] = np.mean(complete_results_copy[6:8])
    complete_results_copy[24] = np.mean(complete_results_copy[21:23])
    print_log_results(placeholder, complete_results_copy[0:15], 'train')
    if validate:
        print_log_results(placeholder, complete_results_copy[15:], 'dev')

    best_scores = record_top_results(complete_results_copy,
                                     best_scores,
                                     placeholder,
                                     False,
                                     threshold)
    #best_scores_all = record_top_results(complete_results,
    #                                     best_scores,
    #                                     placeholder,
    #                                     False)

    return complete_results_copy, best_scores
...
def final_organisation(scores_total, scores_fscore, train_pred, val_pred, df, patience, epoch,
    ...
    """
    main_logger.info(f"Best epoch at: {scores_fscore[-1]}")
    main_logger.info(f"Best Train Acc: {scores_fscore[1]}\nBest Train Fscore:"
                     f" {scores_fscore[4]}\nBest Train Loss: {scores_fscore[7]}")
    if validate:
        main_logger.info(f"Best Val "
                     f"Acc: {scores_fscore[8]}\nBest Val Fscore: {scores_fscore[11]}\nBest "
                     f"Val Loss: {scores_fscore[14]}")

    main_logger.info(f"\nscores: {scores_fscore[1:-1]}")
    ...
    util.save_model_outputs_total_fscore(model_dir,
                            df,
                            train_pred,
                            val_pred,
                            scores_total,
...
def get_output_from_model(model, data):
    ...
    output_pred = model(current_data)
    if cuda:
...
def bookeeping():
    ...
    Outputs:
        ...
        run-throughs
    """
    files_total = [os.path.join(current_dir, 'model', str(f), 'best_scores_total.pickle')
             for f in range(1, exp_runthrough+1)]
    files_fscore = [os.path.join(current_dir, 'model', str(f), 'best_scores_fscore.pickle')
             for f in range(1, exp_runthrough+1)]

    final_results_total = np.zeros((exp_runthrough, 14))
    final_results_fscore = np.zeros((exp_runthrough, 14))
    for f in range(len(files_total)):
        with open(files_total[f], 'rb') as file:
            data_total = pickle.load(file)
        with open(files_fscore[f], 'rb') as file:
            data_fscore = pickle.load(file)
        current_model_dir = os.path.join(current_dir, 'model', str(f+1))
        if data_total[0] == 0:
            pass
        else:
            import pdb
            path_total = os.path.join(current_model_dir,
                                'md_'+str(data_total[-1])+'_epochs.pth')
            path_fscore = os.path.join(current_model_dir, 
                                'md_'+str(data_fscore[-1])+'_epochs.pth')
            mod_paths = [os.path.join(current_model_dir, l) for l in os.listdir(
                current_model_dir) if 'md_' in l]
            del mod_paths[mod_paths.index(path_total)]
            if data_total[-1] != data_fscore[-1]:
                del mod_paths[mod_paths.index(path_fscore)]

            for m in mod_paths:
                os.remove(m)
            try:
                final_results_fscore[f, :] = data_total[0:-1]
            except:
                import pdb
                pdb.set_trace()
    return final_results_fscore
...
def train(model, workspace_files_dir):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=config.EXPERIMENT_DETAILS['WEIGHT_DECAY'])
    ...
    if checkpoint:
        ...
        # total, train_acc, train_fscore, train_loss, val_acc, val_fscore, val_loss, epoch
        best_scores_total = [0] * 16
        best_scores_fscore = [0] * 16
    ...
    gen, train_info, train_folders = organiser.run_train(config,
                                          main_logger,
                                          checkpoint,
                                          features_dir,
                                          data_saver)
    train_folders = list(set(train_folders))
    train_folders.sort()                                      
    if checkpoint:
    ...
    # Train/Val, Accuracy, Precision, Recall, Fscore, Loss(single), mean_acc/f, tn_fp_fn_tp: length 15 each set
    ...
    for (iteration, (batch, train_batch_loc, data_saver)) in enumerate(
        ...
        if begin_evaluate:
            ...
            #best_scores: total, [(train/val) acc_mean, acc_ND, acc_D, f1(avg), f1_ND, f1_D, loss], epoch: length 16
            complete_results_1, best_scores_total = \
            ...
            complete_results_1, best_scores_fscore = \
                update_complete_results(complete_results,
                                        avg_counter,
                                        placeholder,
                                        best_scores_fscore,
            ...
            avg_counter = 0
            complete_results = complete_results_1
            ...
            if placeholder % patience == 0:
                ...
                if not trial < reference:
                    print(f"Val_loss - Patience {reference}\nVal loss - "
                          f"Current {trial}")
                    final_organisation(best_scores_total,
                                       best_scores_fscore,
                                       comp_train_pred,
                                       comp_val_pred,
                                       df,
...
def test():
    ...
    for exp_num in range(exp_runthrough):
        ...
        if threshold is not None:
            ...
            if threshold == 'total':
                with open(model_dir+'/best_scores_total.pickle','rb') as f:
                    best_epoch = pickle.load(f)[-1]
                    model_name = 'md_'+str(best_epoch)+'_epochs.pth'
            ...
            model_name = 'md_'+str(model_index_list[exp_num]+'_epochs.pth')
            model_dir = os.path.join(model_dir, model_name)
            best_epoch = int(model_index_list[exp_num])
        ...
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            scores, per_epoch, results_dict = evaluate_val_test(model,
                                         generator,
                                         data_type,
                                         cw[-3],
                                         np.zeros(15),
                                         ...
                                         gender_balance,
                                         hidden_test)
            results_dict_dir = os.path.join(current_dir, 'results_dict_'+args.threshold)
            if not os.path.exists(results_dict_dir):
                os.mkdir(results_dict_dir)
            with open(os.path.join(results_dict_dir, str(exp_num)+'.pickle'),'wb') as f:
    ...
    print(f"\nAverage Raw:")
    print('Acc(avg): {:.4f}, Acc(ND): {:.4f}, Acc(D): {:.4f}\n F1(avg): {:.4f}, F1(ND): {:.4f}, F1(D): {:.4f}\n tn: {}, fp: {}\n fn: {}, tp: {}'.format(comp_scores_avg[0],\
comp_scores_avg[1],comp_scores_avg[2],comp_scores_avg[3],comp_scores_avg[4],comp_scores_avg[5],comp_scores_avg[6],comp_scores_avg[7],comp_scores_avg[8],comp_scores_avg[9],))
    main_logger.info(f"Average Scores Raw: \n{comp_scores_avg}")
    ...
    if data_type == 'test' or data_type=='dev':
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            comp_scores = evaluation_for_test(results,
                                              ...
                                              main_logger,
                                              hidden_test)
            print('Acc(avg): {:.4f}, Acc(ND): {:.4f}, Acc(D): {:.4f}\n F1(avg): {:.4f}, F1(ND): {:.4f}, F1(D): {:.4f}\n tn: {}, fp: {}\n fn: {}, tp: {}'.format(comp_scores[0],\
comp_scores[1],comp_scores[2],comp_scores[3],comp_scores[4],comp_scores[5],comp_scores[6],comp_scores[7],comp_scores[8],comp_scores[9],))
...
if __name__ == '__main__':
    ...
    parser_test.add_argument('--threshold',
                             type=str,
                             default=None,
                             help="total or fscore")
    parser_test.add_argument('--model_index',
                             type=str,
                             default='1,1,1,1,1')
    ...
    if mode == 'test':
        ...
        model_indexs = args.model_index
        model_index_list = model_indexs.split(',')
    ...
    if args.mode == 'train':
        for i in range(exp_runthrough):
            ...
            next_exp = setup(
                ...
                0)
            if i == 0:
                copy_tree(config.WORKSPACE_FILES_DIR, current_dir+'/workdir/')
