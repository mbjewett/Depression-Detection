...
def save_model_outputs_total_fscore(model_dir, dataframe, train_pred, val_pred,
                       best_scores_total, best_scores_fscore, data_saver):
    """
    Saves the outputs of a model for checkpointing or future analysis for a
    completed experiment.
    ...
    save_path = os.path.join(model_dir, 'best_scores_total.pickle')
    with open(save_path, 'wb') as f:
        pickle.dump((best_scores_total[1:]), f)
    
    save_path = os.path.join(model_dir, 'best_scores_fscore.pickle')
    with open(save_path, 'wb') as f:
        pickle.dump((best_scores_fscore[1:]), f)

    save_out_path = os.path.join(model_dir, 'data_saver.pickle')
    with open(save_out_path, 'wb') as f:
        pickle.dump(data_saver, f)

def load_model_IDL(checkpoint_path, model, optimizer, lr, config):
    import pdb
    checkpoint = torch.load(checkpoint_path)
    pretrained_dict = checkpoint['state_dict']
    pretrained_name = pretrained_dict.keys()
    model_dict = model.state_dict()
    cnt = 0
    for name, param in model.named_parameters():
        if config.batch_norm:
            if name in pretrained_name:
                if param.shape == pretrained_dict[name].shape:
                    param.data.copy_(pretrained_dict[name])
                if config.freeze:
                    param.requires_grad = False
        else:
            if name in pretrained_name and name.find('bn') == -1:# and name.find('running') == -1:
                if param.shape == pretrained_dict[name].shape:
                    param.data.copy_(pretrained_dict[name])
                if config.freeze:
                    param.requires_grad = False
    ...
    torch.set_rng_state(checkpoint['rng_state'])
    torch.cuda.set_rng_state(checkpoint['cuda_rng_state'])
    data_saver_name = 'data_saver_' + str(epoch) + '.pickle'
    data_saver_path = checkpoint_path.split('/')
    data_saver_path = checkpoint_path.replace(data_saver_path[-1],data_saver_name)
    with open(data_saver_path,'rb') as f:
        data_saver = pickle.load(f)

    if config.output_layer_lr is not None:
        my_list = ['fc.fc.weight','fc.fc.bias']
        params = list(map(lambda x: x[1],list(filter(lambda kv: kv[0] in my_list, model.named_parameters()))))
        base_params = list(map(lambda x: x[1],list(filter(lambda kv: kv[0] not in my_list, model.named_parameters()))))
        optimizer = torch.optim.Adam([{'params':base_params},{'params':params,'lr': config.output_layer_lr}], lr = lr)
    
    return model, data_saver, optimizer
...
def eatd_csv_read(file,start=None,end=None):
    print("Reading", file)
    data = []
    all_eatd_df = pd.read_csv(file,sep='\t',header='infer')
    pid_list = all_eatd_df['PID'].tolist()
    clss_list = all_eatd_df['LABEL'].tolist()
    scre_list = all_eatd_df['SDS'].tolist()
    gender_list = all_eatd_df['GENDER'].tolist()
