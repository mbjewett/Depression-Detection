...
def eatd_max_min_values(current_directory, win_size, hop_size, audio_paths, mode_for_background):
    """
...
def eatd_create_database(labels, audio_paths, sample_rate, total_windows_in_file_max, max_value,
    ...
    num_samples_feature = []
    for pointer, filename in enumerate(audio_paths):

        # if 't_' in spk:
        #     spkid = str(ord('t')+ord('_'))+spk.replace('t_','')
        # else:
        #     spkid = str(ord('v')+ord('_'))+spk.replace('v_','')
        
        filename = config.PATH_OF_FOLDER+filename
        spk = filename.split('/')[-2]
        pid = spk
        feat_name= pid+'.npy'
        audio_file_path = os.path.join(current_directory, config.FEATURE_FOLDERS[0],feat_name)
       ...
       # Log mel spectrogram
        if features_exp == 'logmel' or features_exp == 'mel':
            ...
            print('File Name: ', filename, ' dimensions are: ', mel_spec.shape[
                0], mel_spec.shape[1])
            main_logger.info(
                f"Successfully created {features_exp} spectrogram for the "
                f"audio file at: {filename}, it's dimensions "
                f"are: {mel_spec.shape[0]}, "
                f"{mel_spec.shape[1]}")
            height, width = mel_spec.shape #height is number of features and width should be number of frames. 
            num_samples_feature.append(width)
            new_length = width * height
            feat_reshaped = np.reshape(mel_spec, new_length)

        if features_exp =='mfcc':
             feat_path = config.ACOUSTIC_FEATURE+'/'+pid+'.csv'
             feat_df = pd.read_csv(feat_path, sep=',') # change separater to match your file type.
             # Make sure feat_df only has a matrix that you want to use.
             # For example, if you want to drop some feature names: 
             #feat_df = feat_df.drop(['name', 'frameTime'], axis=1)
             #feat_df = feat_df.drop(feat_df.filter(regex='_de').columns, axis=1) 
             feat_mat = feat_df.values
             height, width = feat_mat.shape #height is number of features and width should be number of frames.
             num_samples_feature.append(width)
             new_length = width * height
             feat_reshaped = np.reshape(feat_mat, new_length)
...
def process_organise_data(main_logger,
    ...
    all_split_path = config.ALL_SPLIT_PATH
    ...
    svn = config.EXPERIMENT_DETAILS['SNV']
    ...
    folder_list, audio_paths = fa.eatd_get_meta_data(all_split_path)

    max_value, min_value, sample_rate, total_windows_in_file_max, \
    total_windows_in_file_min, output_data = eatd_max_min_values(current_directory, 
    win_size, hop_size, audio_paths, mode_for_background)
    ...
    labels = utilities.eatd_get_labels_from_dataframe(all_split_path)
    ...
    if config.GENDER:
        ...
        gender = ['f', 'm']
        for i in range(2):
            num_samples_feature =  eatd_create_database(audio_paths, sample_rate,
                                              total_windows_in_file_max,
                                              max_value, current_directory,
                                              features_exp, win_size,
                                              hop_size, svn, freq_bins,
                                              main_logger, whole_train) #not implemented

    else:
        num_samples_feature = eatd_create_database(labels, audio_paths, sample_rate,
                                              total_windows_in_file_max,
                                              max_value, current_directory,
                                              features_exp, win_size,
                                              hop_size, svn, freq_bins,
