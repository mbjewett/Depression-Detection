...
def converge_get_meta_data(all_split_path):

    folder_list = []
    audio_paths = []

    all_converge_df = pd.read_csv(all_split_path,sep='\t',header='infer')
    folder_list = all_converge_df['pid'].tolist()
    list_dir_dataset_path = all_converge_df['path'].tolist()

    for i in list_dir_dataset_path:
        for j in os.listdir(i):
            if 'wav' in j or 'WAV' in j:
                    audio_paths.append(os.path.join(i,j))

    return folder_list, audio_paths

def eatd_get_meta_data(all_split_path):

    folder_list = []
    audio_paths = []

    all_eatd_df = pd.read_csv(all_split_path,sep='\t',header='infer')
    folder_list = all_eatd_df['PID'].tolist()
    audio_paths = all_eatd_df['PATH'].tolist()
