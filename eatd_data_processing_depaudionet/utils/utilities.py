...
def converge_get_labels_from_dataframe(audio_paths):
    """
    Reads database labels from csv file using pandas.
    ...
    for idx,filename in enumerate(audio_paths):
        label = 1 if '/cases/' in filename else 0
        spkid = filename.split('/')[-1][:-4]
        pid = int(spkid)
        pid_array.append(pid)
        binary_array.append(label)
        score_array.append(15) #randome number
        gender_array.append('1') # all feamsles
    output = [pid_array,binary_array,score_array,gender_array]
    return output

def eatd_get_labels_from_dataframe(path):
    """
    Reads database labels from csv file using pandas.
    ...
    output = [df['PID'].values.tolist(),
              df['LABEL'].values.tolist(),
              df['SDS'].values.tolist(),
               df['GENDER'].values.tolist()]
    return output
